    ...
    colorGenetics: 'average' // שונה מ-'randomMix'
      ...
      function getCurrentState() {
        return { 
          ...
          selectedColor,
          isRainbowModeActive,
    tiles: tiles().map(el => getIndex(el)) 
        };
      }
...
state.tiles.forEach((tileState, i) => {
  if (tileElements[i]) {
    ...
    // קובע את אינדקס הצבע 'k' ללא תלות בפורמט
    const k = (typeof tileState === 'object' && tileState !== null) 
              ? tileState.k 
              : tileState;

    // תמיד מסיר את שכבת הזהב וקובע את הצבע לפי האינדקס
