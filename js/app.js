...
function applyState(state) {
    ...
    // The following lines that reset tool states have been removed.
    ...
    // We still call the UI updates in case the palette changed,
    // which can affect the color picker button's appearance.
    updateGlowEffect();
...
longPressTimer = setTimeout(() => {
              wasLongPress = true;
          // --- START: Added code for dark color selection ---
          if (btn.id === 'btnColorPicker') {
              const darkestColor = palette()[0]; // The darkest color is always at index 0
              selectedColor = darkestColor;
              selectedColorIndex = 0;
              isRainbowModeActive = false;
              updateGlowEffect();
              updateColorPickerButtonUI();
              return; // Exit to prevent default long-press behavior
          }
